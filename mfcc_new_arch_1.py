...
def combine_feature():
    ...
    original_feature = load_sound_files(sounds_file_original[:5000]).reshape(5000, 16)
    ...
    fake_feature = load_sound_files(sounds_file_fake[:5000]).reshape(5000, 16)
    ...
    X_train = shuffle_feature[0:5000].reshape(5000, 4, 4, 1)
    x_test = shuffle_feature[5000:].reshape(5000, 4, 4, 1)
    ...
    new_x_train = shuffle_feature[0:5000].reshape(5000, 16)
    new_x_test = shuffle_feature[5000:].reshape(5000, 16)
...
def model():
    ...
    model.add(layers.Conv2D(32, (2, 2),input_shape=(4,4, 1)))
    ...
    model.summary()
    sgd = tf.compat.v2.keras.optimizers.Adadelta(lr=0.0001, rho=0.95, epsilon = 1e-06)
