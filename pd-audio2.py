...
def combine_feature():
    ...
    print(norm_original_feature.shape)
    original_feature = norm_original_feature.reshape(8000, 8, 5, 1)

    Data_dir2 = np.asarray(glob2.glob("/home/shul/audio/fakeData/*"))
    fake_class = []
    label_name2 = []
    fake_feature = []
    j = 0
    ...
    sounds_file_fake = glob2.glob("/home/shul/audio/fakeData/*.wav", recursive=True)
    fake_feature = load_sound_files(sounds_file_fake[:3000])
    norm_fake_feature = preprocessing.normalize(fake_feature, norm='l2')

    fake_feature = norm_fake_feature.reshape(3000, 8, 5, 1)
    ...
    X_train = shuffle_feature[0:9000].reshape(9000, 8, 5, 1)
    x_test = shuffle_feature[9000:].reshape(2000, 8, 5, 1)
    Y_train = shuffle_label[0:9000]
    y_test = shuffle_label[9000:]

    new_x_train = shuffle_feature[0:9000].reshape(9000, 40)
    new_x_test = shuffle_feature[9000:].reshape(2000, 40)
    new_Y_train = shuffle_label[0:9000]
    new_y_test = shuffle_label[9000:]
...
def model():
    ...
    sgd = tf.compat.v2.keras.optimizers.SGD(lr=0.0001, decay = 1e-6, nesterov=False)
    model.compile(loss =  'sparse_categorical_crossentropy', optimizer = "Adam", metrics = ['accuracy'])

    model.fit(X_train, Y_train, validation_split=0.1, shuffle=True, verbose = 2, batch_size=batch_size, epochs = epochs)
