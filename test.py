...
def get_model():
    ...
    #     model.fit(x_train[train], Y_train[train], validation_split=0.1, shuffle=True, verbose = 2, batch_size=batch_size, epochs = epochs)
    ...
    adam = tf.keras.optimizers.Adam(lr=0.01)
    model.compile(loss =  'sparse_categorical_crossentropy', optimizer = adam, metrics = ['accuracy'])
    model.fit(x_train, Y_train, validation_split=0.1, shuffle=True, verbose = 1, batch_size=64, epochs = epochs)
...
tune_search = TuneSearchCV(get_model(),
